...
function show_help() {
    ...
    echo
    echo "  -m  Optional: The valid mode types are:[upgradeOperator], [upgradeOperatorStatus], [upgradeDeployment], [upgradeDeploymentStatus]."
    echo
    ...
    echo "      - STEP 1 (Required): Run the script in [upgradeOperator] mode to upgrade CP4BA operators/migrate the IBM Cloud Pak foundational services and then shutdown all CP4BA operators before upgrade CP4BA deployment.This step will also include the Postgres EDB to CNPG migration if required."
...
function load_temp_property_file(){
    ...
    # load external PostgreSQL for ADPGG/DICMS flag
    tmp_external_pg_flag=$(prop_tmp_property_file EXTERNAL_POSTGRESDB_FOR_ADPGG_DICMS_FLAG)
    if [[ $tmp_external_pg_flag == "true" || $tmp_external_pg_flag == "yes" || $tmp_external_pg_flag == "y" ]]; then
...
# This function support mutiple checkbox, if do not select anything, it will return None
    ...
    content_deployed_tips="\x1B[33;5m[ATTENTION]: \x1B[0m\x1B[1;31m\"Content Cortex Essentials\" can not be selected because one Content (Kind: content.icp4a.ibm.com) custom resource was deployed.\n\x1B[0m"
    indexof() {
        i=-1
...
function select_iam_default_admin() {
  ...
  while true; do
    ...
    case "$ans" in
      "y"|"Y"|"yes"|"Yes"|"YES"|"")
        # For no LDAP with WFPS Authoring, or External IDP, we don't need to check if the user is present in the LDAP
        if [[ "$LDAP_WFPS_AUTHORING" != "no" && "$LDAP_TYPE" != "EXTERNAL_IDP" ]]; then
...
function merge_pattern(){
    ...
    # Only add LDAP sections if LDAP_WFPS_AUTHORING is not "no" and not External IDP
    if [[ "$LDAP_WFPS_AUTHORING" != "no" && "$LDAP_TYPE" != "EXTERNAL_IDP" ]]; then
...
function sync_property_into_final_cr(){
    ...
    # For starter deployments, default to non-production if property not set
    if [[ "$DEPLOYMENT_TYPE" == "starter" && ("$tmp_value" == "<Required>" || -z "$tmp_value") ]]; then
        tmp_value="non-production"
    fi
    ...
    if [[ $odm_Val -eq 0 ]]; then
        ...
        if [[ $DB_TYPE == "postgresql-edb" ]]; then
            ...
            if [[ ! -z "$ds_cfg_val" ]]; then
                ...
                # DBACLD-240890: For CNPG, the DB SSL secret is generated by CP4BA Operator with name {{ meta.name }}-pg-client-cert-secret
                ${YQ_CMD} -i '.spec.datasource_configuration.dc_odm_datasource.dc_ssl_secret_name = "{{ meta.name }}-pg-client-cert-secret"' ${CP4A_PATTERN_FILE_TMP}
            fi
        fi
        ...
        if [[ $DB_TYPE == "db2" || $DB_TYPE == "db2HADR" || $DB_TYPE == "oracle" || $create_odm_postgresql_secret == "Yes" ]]; then
            ...
            if [[ $tmp_ssl_flag == "yes" || $tmp_ssl_flag == "true" || $tmp_ssl_flag == "y" ]]; then
                ...
                # DBACLD-240890: do not set to dc_ssl_secret_name to empty string if CNPG/EDB because we set dc_ssl_secret_name to {{ meta.name }}-pg-client-cert-secret earlier in script
                if [[ $DB_TYPE != "postgresql-edb" ]]; then
    ...
    if [[ "${pattern_cr_arr[@]}" =~ "decisions_ads" ]]; then
        ...
        if [[ "${optional_component_arr[@]}" =~ "DecisionDesigner" ]]; then
            ...
            # Also apply when DB_TYPE is db2/oracle/sqlserver but external PostgreSQL is enabled for ADPGG/DICMS
            if [[ $DB_TYPE = "postgresql" || ( ($DB_TYPE == "db2" || $DB_TYPE == "oracle" || $DB_TYPE == "sqlserver") && "$EXTERNAL_POSTGRESDB_FOR_ADPGG_DICMS" == "true" ) ]]; then
    ...
    if [[ "${pattern_cr_arr[@]}" =~ "decisions_ads" ]]; then
        ...
        if [[ "${optional_component_arr[@]}" =~ "DecisionDesigner" ]]; then
            ...
            if [[ $DB_TYPE != "postgresql" && "$EXTERNAL_POSTGRESDB_FOR_ADPGG_DICMS" != "true" ]]; then
                ${YQ_CMD} -i '.spec.datasource_configuration.dc_ads_designer_datasource = null' ${CP4A_PATTERN_FILE_TMP}
                ${YQ_CMD} -i '.spec.datasource_configuration.dc_ads_designer_datasource.dc_use_postgres = true' ${CP4A_PATTERN_FILE_TMP}
                ${YQ_CMD} -i '.spec.datasource_configuration.dc_ads_designer_datasource.dc_database_type = "postgresql"' ${CP4A_PATTERN_FILE_TMP}
                ${YQ_CMD} -i '.spec.datasource_configuration.dc_ads_designer_datasource.database_servername = "postgres-cp4ba-rw.{{ meta.namespace }}.svc"' ${CP4A_PATTERN_FILE_TMP}
    ...
    # Only process the LDAP configuration if LDAP_WFPS_AUTHORING is not "no" and not External IDP
    if [[ "$LDAP_WFPS_AUTHORING" != "no" && "$LDAP_TYPE" != "EXTERNAL_IDP" ]]; then
...
function apply_pattern_cr(){
   ...
   # Convert optional components array to list by common
    ...
    elif [[ "$DEPLOYMENT_TYPE" == "production" && "$LDAP_TYPE" == "EXTERNAL_IDP" ]]; then
        # Remove ldap_configuration entirely from the CR for External IDP deployments
        ${YQ_CMD} -i 'del(.spec.ldap_configuration)' "${CP4A_PATTERN_FILE_TMP}"
    fi
        ...
        if [[ "$CP4BA_APPLY_CR" == "Yes" || "$CP4BA_APPLY_CR" == "YES" || "$CP4BA_APPLY_CR" == "yes" || "$CP4BA_APPLY_CR" == "True"  || "$CP4BA_APPLY_CR" == "TRUE"  || "$CP4BA_APPLY_CR" == "true" ]]; then
           ...
           if $APPLY_CUSTOM_RESOURCE_CMD ; then
            ...
            if [[ "$UPDATE_COMPONENTS" == "true" ]]; then
                ...
                printf '%b\n' "1. IF you have added any new object stores, you must patch the initialization-config configmap so that the new object stores required for the newly added deployment patterns can be configured. Execute - ${GREEN_TEXT} # ${CLI_CMD} patch cm $cluster_cr_name-initialization-config -n $CP4BA_SERVICES_NS --type merge -p '{\"data\":{\"cpe_initialized\":\"False\"}}' ${RESET_TEXT} and ${GREEN_TEXT} # ${CLI_CMD} patch cm $cluster_cr_name-initialization-config -n $CP4BA_SERVICES_NS --type merge -p '{\"data\":{\"nav_initialized\":\"False\"}}' ${RESET_TEXT}. For more information refer to ${BLUE_TEXT} https://www.ibm.com/docs/en/cloud-paks/cp-biz-automation/$CP4BA_RELEASE_BASE ${RESET_TEXT}.   \n"
...
if [ "$RUNTIME_MODE" == "upgradeOperator" ]; then
    ...
    if [[ "$PLATFORM_SELECTED" == "other" ]]; then
        ...
        #DBACLD-248168: The `top_level_cr_kind` variable is set in retrieve_custom_resource_details function which is called earlier in the `upgradeOperator` mode.
...
if [[ "$RUNTIME_MODE" == "upgradeDeploymentStatus" ]]; then
    ...
    case "$top_level_cr_kind" in
        "content")
            scale_operator "ibm-content-operator" "IBM CP4BA Content Cortex Essentials"
    ...
    # IFix-to-IFix schema persistence for ADPGG external-PG:
    # 26.0.0 GA used "public" as the implicit default schema for ADPGG external-PG.
    # 26.0.0-IF001 changed the default to the DB user schema.
    # A customer who installed at GA and skips directly to IF002+ never ran upgradeDeployment
    # (which is the only place apply_adpgg_schema_from_gitsvc_secret normally fires), so
    # database_schema is never written into the live CR and the new operator default silently
    # breaks ADPGG on the first reconcile.
    # Guard: only when the original install was exactly 26.0.0 GA (cp4ba_original_csv_ver_for_upgrade_script
    # == "26.0.0", no IFix suffix) — IF001 installs already have the correct default so they
    # do not need this patch.
    if [[ "$top_level_cr_kind" == "icp4acluster" \
        && "${CP4BA_RELEASE_BASE}" == "26.0.0" \
        && "$cp4ba_original_csv_ver_for_upgrade_script" == "26.0.0" ]]; then
        _ifix_existing_pattern_list=$(${YQ_CMD} '.spec.shared_configuration.sc_deployment_patterns' "$top_level_cr_details_location" 2>/dev/null)
        _ifix_existing_opt_component_list=$(${YQ_CMD} '.spec.shared_configuration.sc_optional_components' "$top_level_cr_details_location" 2>/dev/null)
        if [[ " $_ifix_existing_pattern_list " =~ "document_processing" && " $_ifix_existing_opt_component_list " =~ "document_processing_designer" ]]; then
            _ifix_dc_use_postgres=$(${YQ_CMD} '.spec.datasource_configuration.dc_adp_datasource.dc_use_postgres' "$top_level_cr_details_location" 2>/dev/null)
            _ifix_dc_schema=$(${YQ_CMD} '.spec.datasource_configuration.dc_adp_datasource.database_schema' "$top_level_cr_details_location" 2>/dev/null)
            if [[ "$_ifix_dc_use_postgres" == "false" && ( -z "$_ifix_dc_schema" || "$_ifix_dc_schema" == "null" ) ]]; then
                info "GA->IFix upgrade detected: dc_adp_datasource.database_schema is absent on the live CR. Patching from ${top_level_cr_name}-gitsvc-secret..."
                patch_adpgg_schema_on_live_cr "$top_level_cr_name" "$CP4BA_SERVICES_NS"
            fi
        fi
    fi
