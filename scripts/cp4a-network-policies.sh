...
function show_help() {
    ...
    echo
    echo "Note:"
    echo "If the existing CP4BA deployment has been installed in \"All Namespaces\" on the cluster, generation or installation of network policies is not permitted for the deployment."
    echo
...
if [[ -z "$TARGET_PROJECT_NAME" ]]; then
    echo -e "\x1B[1;31mPlease rerun command and include value for \"-n <CP4BA_NAMESPACE>\" option.\n\x1B[0m"
    show_help
    exit 1
fi
