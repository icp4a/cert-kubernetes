...
1. The current version of the recipe package `cp4ba-fusion-v0.2.0` supports the following deployment paths and optional components.
...
2. IBM Fusion `v2.12.2` Backup & Restore is the currently the only supported version of IBM Fusion.
...
4. After the restore is complete, apply the following command to make sure nginx is reconfigured and the routes are accessible.
    ...
    ```
    For more information, see [Restore the IBM Cloud Pak for Business Automation platform instance] (https://www.ibm.com/docs/en/fusion-software/2.12.x?topic=machines-restoring-application).
