...
function create_prerequisites() {
    ...
    if [[ " ${pattern_cr_arr[@]}" =~ "document_processing" ]]; then
        ...
        if [[ $tmp_mongo_flag == "Yes" || $tmp_mongo_flag == "YES" || $tmp_mongo_flag == "Y" || $tmp_mongo_flag == "True" || $tmp_mongo_flag == "true" ]]; then
            ...
            ${YQ_CMD} -i ".stringData.mongoUri = \"$tmp_mongo_uri\"" ${ADP_SECRET_FILE}
            # ${SED_COMMAND} "s|# mongoUri:.*|mongoUri: \"$tmp_mongo_uri\"|g" ${ADP_SECRET_FILE}
            ${YQ_CMD} -i ".stringData.mongoUser = \"$tmp_username\"" ${ADP_SECRET_FILE}
            # For https://jsw.ibm.com/browse/DBACLD-157020
            # Function that updates the secret template with the base64 password
...
" "$ADP_SECRET_FILE"
    ...
    if [[ " ${pattern_cr_arr[@]}" =~ "decisions_ads" ]]; then
        ...
        if [[ $tmp_mongo_flag == "Yes" || $tmp_mongo_flag == "YES" || $tmp_mongo_flag == "Y" || $tmp_mongo_flag == "True" || $tmp_mongo_flag == "true" ]]; then
            ...
            ${YQ_CMD} -i ".stringData.gitMongoUri = \"$tmp_uri\"" ${ADS_SECRET_FILE}
            tmp_uri="$(prop_user_profile_property_file ADS.EXTERNAL_MONGO_URI)"
            ${YQ_CMD} -i ".stringData.mongoUri = \"$tmp_uri\"" ${ADS_SECRET_FILE}
            tmp_uri="$(prop_user_profile_property_file ADS.EXTERNAL_MONGO_HISTORY_URI)"
            ${YQ_CMD} -i ".stringData.mongoHistoryUri = \"$tmp_uri\"" ${ADS_SECRET_FILE}
            tmp_uri="$(prop_user_profile_property_file ADS.EXTERNAL_RUNTIME_MONGO_URI)"
            ${YQ_CMD} -i ".stringData.runtimeMongoUri = \"$tmp_uri\"" ${ADS_SECRET_FILE}
...
\\${nl}" ${DB_SERVER_INFO_PROPERTY_FILE}
    ...
    if [[ $EXTERNAL_POSTGRESDB_FOR_IM == "true" ]]; then
        ...
        # Calling generateImZenBTSMessage function from messages.sh to print IM message
    ...
    if [[ $EXTERNAL_POSTGRESDB_FOR_ZEN == "true" ]]; then
        ...
        # Calling generateImZenBTSMessage function from messages.sh to print Zen message
    ...
    if [[ $EXTERNAL_POSTGRESDB_FOR_BTS == "true" ]]; then
        ...
        # Calling generateImZenBTSMessage function from messages.sh to print BTS message
	...
	echo "## Fully Qualified Distinguished Name (FQDN) for the user is required for this setting." >> ${USER_PROFILE_PROPERTY_FILE}
        ...
        echo "" >> ${USER_PROFILE_PROPERTY_FILE}
        echo "## Provide the environment owner password (if password has special characters then xor encoded with {Base64} prefix, otherwise use plain text) for ADP." >> ${USER_PROFILE_PROPERTY_FILE}
        ...
        if [[ " ${optional_component_cr_arr[@]}" =~ "document_processing_runtime" ]]; then
            ...
            echo "## For more information in , from https://www.ibm.com/docs/en/cloud-paks/cp-biz-automation/$CP4BA_RELEASE_BASE, navigate to Document Processing --> Creating a document processing project --> Using feedback documents from applications to improve training" >> ${USER_PROFILE_PROPERTY_FILE}
    ...
    if [[ "${set_scim_attr}" == "true" ]]; then
        ...
        if [[ " ${pattern_cr_arr[@]}" =~ "workflow-runtime" || " ${pattern_cr_arr[@]}" =~ "workflow-authoring" || " ${pattern_cr_arr[@]}" =~ "content" || " ${pattern_cr_arr[@]}" =~ "document_processing" || "${optional_component_cr_arr[@]}" =~ "ae_data_persistence" || (" ${pattern_cr_arr[@]}" =~ "workflow-process-service" && "${optional_component_cr_arr[@]}" =~ "wfps_authoring" && $LDAP_WFPS_AUTHORING == "Yes") ]]; then
            ...
            echo "## https://www.ibm.com/docs/en/cloud-paks/foundational-services/4.6?topic=users-updating-scim-ldap-attributes-mapping#about_ldap_attributes." >> ${USER_PROFILE_PROPERTY_FILE}
            echo "" >> ${USER_PROFILE_PROPERTY_FILE}
...
function select_db_type(){
    ...
    if [[ " ${PATTERNS_CR_SELECTED[@]} " =~ "document_processing" ]]; then
        ...
        options=("IBM Db2 Database" "External PostgreSQL" "EDB Postgres (deployed by the CP4BA Operator)")
        PS3='Enter a valid option [1 to 3]: '
        # else
        ...
        # if [[ $PROFILE_TYPE == "small" ]]; then
        options=("IBM Db2 Database" "Oracle" "Microsoft SQL Server" "External PostgreSQL" "EDB Postgres (deployed by the CP4BA Operator)")
...
function validate_prerequisites(){
    ...
    CP4BA_AUTO_REGION=${CP4BA_AUTO_REGION:-"US"}
