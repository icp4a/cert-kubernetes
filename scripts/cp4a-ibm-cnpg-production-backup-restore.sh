...
# EDB default values
CNPG_CLUSTER_RESOURCE="clusters.pg.ibm.com"
CNPG_API_VERSION="pg.ibm.com/v1"
...
function get_edb_cluster() {
  ...
  ${CLI_CMD} get ${CNPG_CLUSTER_RESOURCE} ${EDB_CLUSTER_NAME} -o yaml > ${EDB_CLUSTER_YAML_FILE}
...
function schedule_backups() {
  ...
  ${YQ_CMD} -i ".apiVersion = \"${CNPG_API_VERSION}\"" $EDB_CLUSTER_SCHEDULE_BACKUP
