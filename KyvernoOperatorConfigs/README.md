...
- `ibm-ads-operator/`: Contains JSON files for patching ADS operator's CSV to support HA, PDB template and ephemeral volume configurations.
...
- `<operator names>` is an optional comma-separated list of operator names to patch. The operator name must match the directory name (eg: ibm-cp4a-operator, ibm-content-operator, etc..). If not provided, all available operators will be patched.
