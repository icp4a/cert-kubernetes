...
patch_fusion_app() {
    ...
    # Check and add ibm-licensing namespace
    local has_licensing_ns
    has_licensing_ns=$($KC -n "$fusion_ns" get fapp/"$cp4ba_ns" -o jsonpath='{.spec.includedNamespaces[?(@=="ibm-licensing")]}' 2>/dev/null)

    if [[ -z "$has_licensing_ns" ]]; then
        log_info "Adding 'ibm-licensing' to Fusion Application includedNamespaces..."

        if [[ "$DRY_RUN" == "true" ]]; then
            log_info "[DRY-RUN] Would add 'ibm-licensing' to fapp/$cp4ba_ns"
        else
            $KC patch fapp "$cp4ba_ns" -n "$fusion_ns" --type json \
                --patch '[{"op":"add","path":"/spec/includedNamespaces/-","value":"ibm-licensing"}]'
            log_success "Added 'ibm-licensing' to includedNamespaces"
        fi
    else
        log_success "'ibm-licensing' already in includedNamespaces"
    fi
}
...
label_core_resources() {
    ...
    label_resource "$cp4ba_ns" "secret" "cp4ba-ums-secret" "cp4ba.ibm.com/backup-type=mandatory"
    label_resource "$cp4ba_ns" "ibmusagemetering" "ibmusagemetering" "cp4ba.ibm.com/backup-type=mandatory"
