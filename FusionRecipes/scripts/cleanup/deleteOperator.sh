    ...
    if [[ $RUNTIME_MODE == "baw" || $RUNTIME_MODE == "baw-dev" ]];then
       ...
       for word in $subsArr
        do
            ...
            echo $word
            # - get csv name
            csvName=$(kubectl get subscription.operators.coreos.com "$word" -n $NAMESPACE -o go-template --template '{{.status.installedCSV}}')
            # - remove the subscription
            echo "removing the subscription for "$word
            kubectl delete subscription.operators.coreos.com $word -n $NAMESPACE >/dev/null 2>&1
            # - remove the CSV
