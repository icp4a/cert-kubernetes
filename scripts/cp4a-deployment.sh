...
function show_help() {
    ...
    echo "      - STEP 1 (Required): Run the script in [upgradeOperator] mode to upgrade CP4BA operators and migrate (Cluster-scoped -> Cluster-scoped [AllNamespaces] / Namespace-scoped -> Namespace-scoped) the IBM Cloud Pak foundational services. Shutdown all CP4BA operators before upgrading the CP4BA deployment.This step will also include the Postgres EDB to CNPG migration if required."
...
#DBACLD-175890: There is no need for CSV version in 25.0.0.x or later.
    ...
    if [[ "$PLATFORM_SELECTED" == "other" ]]; then
        ...
        # WORKAROUND: https://jsw.ibm.com/browse/DBACLD-251141
        ...
        if ${CLI_CMD} get operandconfig common-service -n "$cpfs_services_ns" --no-headers --ignore-not-found >&3 2>&3; then
            _cpfs_patched=$(${CLI_CMD} get operandconfig common-service -n "$cpfs_services_ns" -o yaml | \
              ${YQ_CMD} '(.spec.services[] | select(.resources) | .resources[] | select(.apiVersion == "pg.ibm.com/v1" and .kind == "Cluster" and .name == "common-service-db") | .data.spec.primaryUpdateMethod) = "restart"')
            if [[ -n "$_cpfs_patched" ]]; then
                ...
                if [[ $? -eq 0 ]]; then
                    ...
                    echo "  ${CLI_CMD} get operandconfig common-service -n $cpfs_services_ns -o yaml | ${YQ_CMD} '(.spec.services[] | select(.resources) | .resources[] | select(.apiVersion == \"pg.ibm.com/v1\" and .kind == \"Cluster\" and .name == \"common-service-db\") | .data.spec.primaryUpdateMethod) = \"restart\"' | ${CLI_CMD} apply -f -"
                fi
            else
...
if [[ "$RUNTIME_MODE" == "upgradeDeploymentStatus" ]]; then
    ...
    # Patching operandConfig here ensures the fix is in place before validate_zen_upgrade_status
    # begins monitoring. Remove once the CPFS team ships an official fix.
