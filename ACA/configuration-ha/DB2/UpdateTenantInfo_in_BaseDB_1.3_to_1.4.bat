...
:DOCREATE
	...
	db2 update tenantinfo set TENANTDBVERSION=1.4 where TENANTID='%tenant_id%' and ONTOLOGY='%tenant_ontology%'
	db2 update tenantinfo set FEATUREFLAGS=BITOR(4, (select FEATUREFLAGS from tenantinfo where TENANTID='%tenant_id%' and ONTOLOGY='%tenant_ontology%')) where TENANTID='%tenant_id%' and ONTOLOGY='%tenant_ontology%'
	db2 connect reset
	goto END
