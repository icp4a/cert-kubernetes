...
if [[ $USING_HELM == "y" || $USING_HELM == "yes" ]]; then
    ...
    echo "helm delete celery${KUBE_NAME_SPACE} --purge --tiller-namespace tiller"
    helm delete celery${KUBE_NAME_SPACE} --purge --tiller-namespace tiller
    fi
fi
echo
...
if [[ $ICP_VERSION == "3.1.2" ]]; then
    ...
    kubectl delete -n $KUBE_NAME_SPACE psp baca-anyuid-psp
fi
set -e
