...
function interactive_mode() {
    ...
    local operators_ns=""
    local services_ns=""
    local config_search_ns="$INSTANCE_NAMESPACE"

    # In a separation-of-duties deployment the user may pass the operator namespace
    # via -n instead of the instance (services) namespace.  The ConfigMap lives in
    # the services namespace, so if it is not found in the supplied namespace we
    # fall back to scanning all accessible namespaces for it.
    if ! ${CLI_CMD} get configmap "$config_map_name" -n "$INSTANCE_NAMESPACE" &> /dev/null; then
        ...
        info "Searching other namespaces for CP4BA configuration..."
        local found_ns
        found_ns=$(${CLI_CMD} get configmap "$config_map_name" --all-namespaces \
            -o jsonpath='{.items[0].metadata.namespace}' 2>/dev/null)
        # If --all-namespaces query fails (RBAC restriction), iterate accessible namespaces
        if [ -z "$found_ns" ]; then
            local accessible_ns
            accessible_ns=$(${CLI_CMD} get namespaces -o jsonpath='{.items[*].metadata.name}' 2>/dev/null)
            for ns in $accessible_ns; do
                if [ "$ns" = "$INSTANCE_NAMESPACE" ]; then
                    continue
                fi
                if ${CLI_CMD} get configmap "$config_map_name" -n "$ns" &> /dev/null; then
                    found_ns="$ns"
                    break
                fi
            done
        ...
        if [ -n "$found_ns" ]; then
            info "Found '$config_map_name' in namespace: $found_ns"
            config_search_ns="$found_ns"
        else
            config_search_ns=""
        fi
    fi

    if [ -n "$config_search_ns" ]; then
        operators_ns=$(${CLI_CMD} get configmap "$config_map_name" -n "$config_search_ns" \
            -o jsonpath='{.data.operators_namespace}' 2>/dev/null)
        services_ns=$(${CLI_CMD} get configmap "$config_map_name" -n "$config_search_ns" \
        ...
        if [ -n "$operators_ns" ] && [ -n "$services_ns" ]; then
            ...
            if [ "$operators_ns" != "$services_ns" ]; then
                ...
                INSTANCE_NAMESPACE="$services_ns"
            else
        ...
        warning "ConfigMap '$config_map_name' not found in any accessible namespace"
