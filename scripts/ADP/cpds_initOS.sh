...
{
    echo "NAME"
    echo "  $0 -- initializes the ObjectStore for Project deployment."
    echo ""
    echo "USAGE: $0  "
...
curl -X POST --header "Authorization:Bearer ${RUN_BEARER}" --header 'Content-Type: application/json' --header 'Accept: application/json' -w '\nReturn Code=%{http_code}\n\n' "${runtimeCpdsUrl}/ibm-dba-content-deployment/v1/repositories/${runtimeObjectStore}/initialization" -k
