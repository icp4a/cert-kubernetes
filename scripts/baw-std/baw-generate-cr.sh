...
function func_sync_gcd_ds_property_into_cr(){
    ...
    for i in "${!GCDDB_CR_MAPPING[@]}"; do
        ${YQ_CMD} -i ".${GCDDB_CR_MAPPING[i]} = \"$(prop_db_server_property_file $tmp_gcd_db_servername.${GCDDB_COMMON_PROPERTY[$i]})\"" ${BAW_STD_PATTERN_FILE_TMP}
    done

    # remove database_name if oracle
    if [[ $DB_TYPE == "oracle" ]]; then
        ${YQ_CMD} -i '.spec.datasource_configuration.dc_gcd_datasource.database_name = "<Remove>"' ${BAW_STD_PATTERN_FILE_TMP}
    else
        ${YQ_CMD} -i ".spec.datasource_configuration.dc_gcd_datasource.database_name = \"$tmp_gcd_db_name\"" ${BAW_STD_PATTERN_FILE_TMP}
    fi
}
...
function func_sync_ae_os_ds_property_into_cr(){
    ...
    if [[ -n $OS_DATASOURCE_NUMBER && $OS_DATASOURCE_NUMBER -gt 0 ]]; then
        ...
        for i in "${!OSDB_CR_MAPPING[@]}"; do
            ${YQ_CMD} -i ".spec.datasource_configuration.dc_os_datasources[$OS_DATASOURCE_NUMBER].${OSDB_CR_MAPPING[i]} = \"$(prop_db_server_property_file $tmp_os_db_servername.${OSDB_COMMON_PROPERTY[$i]})\"" ${BAW_STD_PATTERN_FILE_TMP}
        done
        
        ${YQ_CMD} -i ".spec.datasource_configuration.dc_os_datasources[$OS_DATASOURCE_NUMBER].dc_os_label = \"aeos\"" ${BAW_STD_PATTERN_FILE_TMP}
        ${YQ_CMD} -i ".spec.datasource_configuration.dc_os_datasources[$OS_DATASOURCE_NUMBER].database_name = \"$tmp_os_db_name\"" ${BAW_STD_PATTERN_FILE_TMP}
    fi
}
...
function func_sync_icn_ds_property_into_cr(){
    ...
    for i in "${!ICNDB_CR_MAPPING[@]}"; do
        ${YQ_CMD} -i ".${ICNDB_CR_MAPPING[i]} = \"$(prop_db_server_property_file $tmp_icn_db_servername.${ICNDB_COMMON_PROPERTY[$i]})\"" ${BAW_STD_PATTERN_FILE_TMP}
    done
    
    # remove database_name if oracle
    if [[ $DB_TYPE == "oracle" ]]; then
        ${YQ_CMD} -i '.spec.datasource_configuration.dc_icn_datasource.database_name = "<Remove>"' ${BAW_STD_PATTERN_FILE_TMP}
    else
        ${YQ_CMD} -i ".spec.datasource_configuration.dc_icn_datasource.database_name = \"$tmp_icn_db_name\"" ${BAW_STD_PATTERN_FILE_TMP}
    fi
}
...
function func_sync_ums_ds_property_into_cr(){
    ...
    for i in "${!UMSDB_OAUTH_CR_MAPPING[@]}"; do
        ${YQ_CMD} -i ".${UMSDB_OAUTH_CR_MAPPING[i]} = \"$(prop_db_server_property_file $tmp_ums_oauth_db_servername.${UMSDB_OAUTH_COMMON_PROPERTY[$i]})\"" ${BAW_STD_PATTERN_FILE_TMP}
    done
    ${YQ_CMD} -i ".spec.datasource_configuration.dc_ums_datasource.dc_ums_oauth_name = \"$tmp_ums_oauth_db_name\"" ${BAW_STD_PATTERN_FILE_TMP}

    if [[ $DB_TYPE == "oracle" ]]; then
        tmp_ums_oauth_oracle_service_name="$(prop_db_name_user_property_file UMS_DB_SERVICE_NAME)"
        ${YQ_CMD} -i ".spec.datasource_configuration.dc_ums_datasource.dc_ums_oauth_oracle_service_name = \"$tmp_ums_oauth_oracle_service_name\"" ${BAW_STD_PATTERN_FILE_TMP}
    fi
    ...
    for i in "${!UMSDB_TS_CR_MAPPING[@]}"; do
        ${YQ_CMD} -i ".${UMSDB_TS_CR_MAPPING[i]} = \"$(prop_db_server_property_file $tmp_ums_oauth_db_servername.${UMSDB_TS_COMMON_PROPERTY[$i]})\"" ${BAW_STD_PATTERN_FILE_TMP}
    done
    ${YQ_CMD} -i ".spec.datasource_configuration.dc_ums_datasource.dc_ums_teamserver_name = \"$tmp_ums_ts_db_name\"" ${BAW_STD_PATTERN_FILE_TMP}

    if [[ $DB_TYPE == "oracle" ]]; then
        tmp_ums_oauth_oracle_service_name="$(prop_db_name_user_property_file UMS_DB_SERVICE_NAME)"
        ${YQ_CMD} -i ".spec.datasource_configuration.dc_ums_datasource.dc_ums_teamserver_oracle_service_name = \"$tmp_ums_oauth_oracle_service_name\"" ${BAW_STD_PATTERN_FILE_TMP}
    fi
}
...
function func_sync_ldap_property_into_cr(){
    for i in "${!LDAP_COMMON_CR_MAPPING[@]}"; do
        ${YQ_CMD} -i ".${LDAP_COMMON_CR_MAPPING[i]} = \"$(prop_ldap_property_file ${LDAP_COMMON_PROPERTY[$i]})\"" ${BAW_STD_PATTERN_FILE_TMP}
    done
...
function func_sync_baw_std_property_file_into_cr(){
    ...
    for i in "${!BAW_RUNTIME_CR_MAPPING[@]}"; do
        ${YQ_CMD} -i ".${BAW_RUNTIME_CR_MAPPING[i]} = \"$(prop_db_server_property_file $tmp_baw_runtime_db_servername.${BAW_RUNTIME_COMMON_PROPERTY[$i]})\"" ${BAW_STD_PATTERN_FILE_TMP}
    done
    ...
    # remove database_name if oracle
    if [[ $DB_TYPE == "oracle" ]]; then
        ${YQ_CMD} -i '.spec.baw_configuration[0].database.database_name = ""' ${BAW_STD_PATTERN_FILE_TMP}
        ${YQ_CMD} -i '.spec.baw_configuration[0].database.server_name = ""' ${BAW_STD_PATTERN_FILE_TMP}
        ${YQ_CMD} -i '.spec.baw_configuration[0].database.port = ""' ${BAW_STD_PATTERN_FILE_TMP}
    else
        ${YQ_CMD} -i ".spec.baw_configuration.[0].database.database_name = \"$tmp_baw_runtime_db_name\"" ${BAW_STD_PATTERN_FILE_TMP}
    fi
    ...
    if [[ $DB_TYPE == "oracle" ]]; then
        ...
        ${YQ_CMD} -i ".spec.baw_configuration.[0].database.jdbc_url = \"$tmp_baw_runtime_db_jdbc_url\"" ${BAW_STD_PATTERN_FILE_TMP}
    else
        ${YQ_CMD} -i '.spec.baw_configuration[0].database.jdbc_url = "<Remove>"' ${BAW_STD_PATTERN_FILE_TMP}
        ${SED_COMMAND} 's|jdbc_url: "<Remove>"|# jdbc_url: ""|g' ${BAW_STD_PATTERN_FILE_TMP}
    fi
    ${YQ_CMD} -i '.spec.baw_configuration[0].database.custom_jdbc_pvc = "<Remove>"' ${BAW_STD_PATTERN_FILE_TMP}
    ...
    ${YQ_CMD} -i ".spec.baw_configuration.[0].admin_user = \"$tmp_baw_runtime_admin\"" ${BAW_STD_PATTERN_FILE_TMP}
    ...
    if [ -n "$tmp_date_sql" ] && [ -n "$tmp_logical_unique_id" ]; then
        ${YQ_CMD} -i ".spec.baw_configuration.[0].case.event_emitter.[0].date_sql = \"$tmp_date_sql\"" ${BAW_STD_PATTERN_FILE_TMP}
        ${YQ_CMD} -i ".spec.baw_configuration.[0].case.event_emitter.[0].logical_unique_id = \"$tmp_logical_unique_id\"" ${BAW_STD_PATTERN_FILE_TMP}

        # Set connection point name
        ${YQ_CMD} -i ".spec.baw_configuration.[0].case.event_emitter.[0].connection_point_name = \"$tmp_conn_point_name\"" ${BAW_STD_PATTERN_FILE_TMP}
    
    else
        ${YQ_CMD} -i 'del(.spec.baw_configuration.[0].case.event_emitter)' "${BAW_STD_PATTERN_FILE_TMP}"
    fi

    # Set connection point name
    ${YQ_CMD} -i ".spec.baw_configuration.[0].case.tos_list.[0].connection_point_name = \"$tmp_conn_point_name\"" ${BAW_STD_PATTERN_FILE_TMP}
}
...
function func_sync_aae_property_file_into_cr() {
    ...
    for i in "${!AEDB_CR_MAPPING[@]}"; do
        if [[ $DB_TYPE == "oracle" ]]; then
            ${YQ_CMD} -i ".${AEDB_CR_MAPPING[i]} = \"$(prop_db_oracle_server_property_file $tmp_ae_db_servername.${AEDB_COMMON_PROPERTY[$i]})\"" ${BAW_STD_PATTERN_FILE_TMP}
        else
            ${YQ_CMD} -i ".${AEDB_CR_MAPPING[i]} = \"$(prop_db_server_property_file $tmp_ae_db_servername.${AEDB_COMMON_PROPERTY[$i]})\"" ${BAW_STD_PATTERN_FILE_TMP}
        fi
    done
    ...
    # remove database_name if oracle
    if [[ $DB_TYPE == "oracle" ]]; then
        ${YQ_CMD} -i '.spec.application_engine_configuration[0].database.host = ""' ${BAW_STD_PATTERN_FILE_TMP}
        ${YQ_CMD} -i '.spec.application_engine_configuration[0].database.port = ""' ${BAW_STD_PATTERN_FILE_TMP}
        ${YQ_CMD} -i '.spec.application_engine_configuration[0].database.name = ""' ${BAW_STD_PATTERN_FILE_TMP}
    else
        ${YQ_CMD} -i ".spec.application_engine_configuration.[0].database.name = \"$tmp_ae_db_name\"" ${BAW_STD_PATTERN_FILE_TMP}
    fi

    if [[ $DB_TYPE != "oracle" ]]; then
        ${YQ_CMD} -i 'del(.spec.application_engine_configuration.[0].database.oracle_url_without_wallet_directory)' "${BAW_STD_PATTERN_FILE_TMP}"
        ${YQ_CMD} -i 'del(.spec.application_engine_configuration.[0].database.oracle_url_with_wallet_directory)' "${BAW_STD_PATTERN_FILE_TMP}"
        ${YQ_CMD} -i 'del(.spec.application_engine_configuration.[0].database.oracle_sso_wallet_secret_name)' "${BAW_STD_PATTERN_FILE_TMP}"
    fi

    # Applying user profile for AE
    tmp_ae_admin="$(prop_user_profile_property_file APP_ENGINE.ADMIN_USER)"
    ${YQ_CMD} -i ".spec.application_engine_configuration.[0].admin_user = \"$tmp_ae_admin\"" ${BAW_STD_PATTERN_FILE_TMP}
}
...
function func_set_elaticsearch_configuration_in_cr() {
    ...
    es_privileged=`${YQ_CMD} ".spec.elasticsearch_configuration.privileged" "${BAW_STD_PATTERN_FILE_TMP}"`
    
    if [[ $es_privileged == "false" ]]; then
        ${YQ_CMD} -i '.spec.elasticsearch_configuration.service_account = ""' ${BAW_STD_PATTERN_FILE_TMP}
    fi
}
...
function func_set_initialize_configuration_in_cr() {
    ...
    if [[ $tmp_init_flag == "yes" || $tmp_init_flag == "y" || $tmp_init_flag == "true" ]]; then
        ${YQ_CMD} -i '.spec.shared_configuration.sc_content_initialization = true' ${BAW_STD_PATTERN_FILE_TMP}
        ...
        for num in "${!ldap_admin_user_name_array[@]}"; do
            ...
            ${YQ_CMD} -i ".spec.initialize_configuration.ic_ldap_creation.ic_ldap_admin_user_name[$((num))] = \"${tmp_admin_user_name}\"" ${BAW_STD_PATTERN_FILE_TMP}
        done

        for num in "${!ldap_admins_groups_name_array[@]}"; do
            local tmp_admins_groups_name=$(sed -e 's/^[[:space:]]*//' <<< "${ldap_admins_groups_name_array[num]}" )
            ${YQ_CMD} -i ".spec.initialize_configuration.ic_ldap_creation.ic_ldap_admins_groups_name[$((num))] = \"${tmp_admins_groups_name}\"" ${BAW_STD_PATTERN_FILE_TMP}
        done
        ...
        for i in "${!BAW_RUNTIME_OS_ARR[@]}"; do
            ...
            if [[ -n $OS_DATASOURCE_NUMBER && $OS_DATASOURCE_NUMBER -gt 0 ]]; then
                ...
                for num in "${!admin_user_group_array[@]}"; do
                    ...
                    ${YQ_CMD} -i ".spec.initialize_configuration.ic_obj_store_creation.object_stores[$OS_DATASOURCE_NUMBER].oc_cpe_obj_store_admin_user_groups[$((num))] = \"${tmp_admin_user_group_name}\"" ${BAW_STD_PATTERN_FILE_TMP}
                done
            fi
            ...
            if [[ "${BAW_RUNTIME_OS_ARR[i]}" == "BAWINS1TOS" ]]; then
                ...
                if [[ $tmp_workflow_flag == "yes" || $tmp_workflow_flag == "y" || $tmp_workflow_flag == "true" ]]; then
                    ...
                    ${YQ_CMD} -i ".spec.initialize_configuration.ic_obj_store_creation.object_stores.[$OS_DATASOURCE_NUMBER].oc_cpe_obj_store_workflow_data_tbl_space = \"$tmp_val\"" ${BAW_STD_PATTERN_FILE_TMP}

                    tmp_val=$(prop_user_profile_property_file CONTENT_INITIALIZATION.CPE_OBJ_STORE_WORKFLOW_ADMIN_GROUP)
                    tmp_val=$(sed -e 's/^"//' -e 's/"$//' <<<"$tmp_val")
                    ${YQ_CMD} -i ".spec.initialize_configuration.ic_obj_store_creation.object_stores.[$OS_DATASOURCE_NUMBER].oc_cpe_obj_store_workflow_admin_group = \"$tmp_val\"" ${BAW_STD_PATTERN_FILE_TMP}

                    tmp_val=$(prop_user_profile_property_file CONTENT_INITIALIZATION.CPE_OBJ_STORE_WORKFLOW_CONFIG_GROUP)
                    tmp_val=$(sed -e 's/^"//' -e 's/"$//' <<<"$tmp_val")
                    ${YQ_CMD} -i ".spec.initialize_configuration.ic_obj_store_creation.object_stores.[$OS_DATASOURCE_NUMBER].oc_cpe_obj_store_workflow_config_group = \"$tmp_val\"" ${BAW_STD_PATTERN_FILE_TMP}

                    tmp_val=$(prop_user_profile_property_file CONTENT_INITIALIZATION.CPE_OBJ_STORE_WORKFLOW_PE_CONN_POINT_NAME)
                    tmp_val=$(sed -e 's/^"//' -e 's/"$//' <<<"$tmp_val")
                    ${YQ_CMD} -i ".spec.initialize_configuration.ic_obj_store_creation.object_stores.[$OS_DATASOURCE_NUMBER].oc_cpe_obj_store_workflow_pe_conn_point_name = \"$tmp_val\"" ${BAW_STD_PATTERN_FILE_TMP}
                else
                    ${YQ_CMD} -i ".spec.initialize_configuration.ic_obj_store_creation.object_stores[$OS_DATASOURCE_NUMBER].oc_cpe_obj_store_enable_workflow = \"false\"" ${BAW_STD_PATTERN_FILE_TMP}
                    ${YQ_CMD} -i ".spec.initialize_configuration.ic_obj_store_creation.object_stores[$OS_DATASOURCE_NUMBER].oc_cpe_obj_store_workflow_data_tbl_space = \"\"" ${BAW_STD_PATTERN_FILE_TMP}
                fi
            fi
        done
        ...
        ${YQ_CMD} -i '.spec.shared_configuration.sc_content_initialization = false' ${BAW_STD_PATTERN_FILE_TMP}
    fi
}
...
function generate_baw_std_cr_file(){
    ...
    # convert license
    ${YQ_CMD} -i '.spec.ibm_license = "accept"' ${BAW_STD_PATTERN_FILE_TMP}
    ...
    ${YQ_CMD} -i ".spec.shared_configuration.sc_deployment_baw_license = \"$tmp_license_baw\"" ${BAW_STD_PATTERN_FILE_TMP}
    ${YQ_CMD} -i ".spec.shared_configuration.sc_deployment_fncm_license = \"$tmp_license_fncm\"" ${BAW_STD_PATTERN_FILE_TMP}
    if [[ -n "$tmp_license_cp4ba" ]]; then
        ${YQ_CMD} -i ".spec.shared_configuration.sc_deployment_license = \"$tmp_license_cp4ba\"" ${BAW_STD_PATTERN_FILE_TMP}
    else
        ${YQ_CMD} -i '.spec.shared_configuration.sc_deployment_license = "production"' ${BAW_STD_PATTERN_FILE_TMP}
    fi
    
    # Set sc_deployment_context
    tmp_deployment_context=$(prop_user_profile_property_file CP4BA.PURCHASED_PRODUCT | tr '[:lower:]' '[:upper:]')
    ${YQ_CMD} -i ".spec.shared_configuration.sc_deployment_context = \"$tmp_deployment_context\"" ${BAW_STD_PATTERN_FILE_TMP}
    ...
    ${YQ_CMD} -i ".spec.shared_configuration.sc_deployment_hostname_suffix = \"${SC_DEPLOYMENT_HOSTNAME_SUFFIX}\"" ${BAW_STD_PATTERN_FILE_TMP}
    ...
    if [[ (! -z $generate_network_policy_flag) && $generate_network_policy_flag == "true" ]]; then
        ${YQ_CMD} -i '.spec.shared_configuration.sc_generate_sample_network_policies = true' ${BAW_STD_PATTERN_FILE_TMP}
    else
        ${YQ_CMD} -i '.spec.shared_configuration.sc_generate_sample_network_policies = false' ${BAW_STD_PATTERN_FILE_TMP}
    fi
    ...
    # Others convert
    ${YQ_CMD} -i 'del(.null)' "${BAW_STD_PATTERN_FILE_TMP}"
