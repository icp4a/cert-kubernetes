...
do
   ssh $SSH_USER@$NFS_IP -oStrictHostKeyChecking=no "$SUDO_CMD mkdir -p /exports/smartpages/$KUBE_NAME_SPACE/configdb-${i}"
done
...
do
    ...
    do
         ssh $SSH_USER@$NFS_IP -oStrictHostKeyChecking=no "$SUDO_CMD mkdir -p /exports/smartpages/$KUBE_NAME_SPACE/mongodb-shard${i}-${j}"
    done
done
...
# !!!Replicas if your mongodb-configdb has more than x>=3 replicas, please add mongodb-configdb-{x-1}.mongodb-configdb-service.${KUBE_NAME_SPACE}.svc.cluster.local:27019 in the end 
CONFIG_REPLSET_VALUE="configReplSet/mongodb-configdb-0.mongodb-configdb-service.sp.svc.cluster.local:27019,mongodb-configdb-1.mongodb-configdb-service.sp.svc.cluster.local:27019,mongodb-configdb-2.mongodb-configdb-service.sp.svc.cluster.local:27019"
