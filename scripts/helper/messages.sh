...
function next_steps_for_major_upgrade_after_upgrade_operator_mode() {
  ...
  step_num=1
  echo "  - STEP ${step_num} ${YELLOW_TEXT}(Optional)${RESET_TEXT}: You can run ${GREEN_TEXT}\"${cur_dir}/cp4a-deployment.sh -m upgradeOperatorStatus -n $namespace\"${RESET_TEXT} to check whether the upgrade of the CP4BA operator and its dependencies is successful."
  step_num=$((step_num + 1))
