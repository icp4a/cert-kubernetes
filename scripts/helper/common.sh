...
MINIMUM_SUPPORTED_UPGRADE_VERSIONS=("24.0.0")
...
function validate_java_runtime() {
    ...
    if [[ -n "$CUSTOM_JAVA_PATH" ]]; then
        ...
        if [[ ! -x "$JAVA_CMD" ]]; then
            ...
            printf '%b\n' "\x1B[1;31mPlease provide a valid path to Java (JRE) $REQUIRED_JAVA_MAJOR_VERSION or higherinstallation.\x1B[0m"
            exit 1
        fi
...
# If the upgrade is from 22.0.2 then there is strimzipodset resource , instead the stateful set resource must be patched -> https://jsw.ibm.com/browse/DBACLD-210631
...
function patch_strimzi_podset(){
    ...
    if [[ "$events_operator_channel" == "v5.2" ]]; then
        ...
        # Check for StrimziPodSet resource first
        ...
        if [[ -n "$kafka_podset_exists" ]]; then
            ...
            # Apply the patch to StrimziPodSet
            echo "Applying patch to StrimziPodSet to update annotations..."
            ...
            echo "Successfully updated StrimziPodSet annotations:"
        ...
        # https://jsw.ibm.com/browse/DBACLD-210631
            ...
            echo "StrimziPodSet 'iaf-system-kafka' not found, checking for StatefulSet..."
            
            # Check for StatefulSet
            kafka_statefulset_exists=$(${CLI_CMD} get statefulset iaf-system-kafka -n $services_namespace -o name --no-headers 2>/dev/null || echo "")

            if [[ -n "$kafka_statefulset_exists" ]]; then
                echo "Found StatefulSet 'iaf-system-kafka'"

                # Get the current kafka version from the StatefulSet annotation
                kafka_annotation_value=$(${CLI_CMD} get statefulset iaf-system-kafka -n $services_namespace -o yaml | ${YQ_CMD} '.metadata.annotations."strimzi.io/kafka-version"')
                ...
                # Apply the patch to StatefulSet (both metadata and template.metadata annotations)
                echo "Applying patch to StatefulSet to update annotations..."
                ${CLI_CMD} patch statefulset iaf-system-kafka -n $services_namespace --type=merge -p "{\"metadata\":{\"annotations\":{\"strimzi.io/kafka-version\":null,\"ibmevents.ibm.com/kafka-version\":\"$kafka_annotation_value\"}},\"spec\":{\"template\":{\"metadata\":{\"annotations\":{\"strimzi.io/kafka-version\":null,\"ibmevents.ibm.com/kafka-version\":\"$kafka_annotation_value\"}}}}}"

                echo "Successfully updated StatefulSet annotations:"
